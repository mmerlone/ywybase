...
import { SOCIAL_LINK_CARD_HEIGHT, SocialLinkCard } from './SocialLinkCard'
...
/**
 * TabSocialLinks - Dynamic array of social link cards with inline add/edit forms and OG metadata fetching.
 *
 * @remarks
 * Features:
 ...
 *
 * @returns ReactElement - The social links tab component
 */
export function TabSocialLinks(): ReactElement {
  ...
  const fetchMetadata = useCallback(
    async (url: string): Promise<OgMeta | null> => {
      ...
      try {
        ...
        const normalizedErr = err instanceof Error ? err : new Error('Unknown error')
        logger.error({ error: normalizedErr, url }, 'Failed to fetch metadata')
        showError(normalizedErr.message)
        setMetaPreview(null)
        return null
      } finally {
  ...
  const handleAdd = useCallback(
    async (data: SocialLinkFormData): Promise<void> => {
      try {
        ...
        const normalizedErr = err instanceof Error ? err : new Error('Unknown error')
        logger.error({ error: normalizedErr, data }, 'Failed to add social link')
        showError(`Failed to add link: ${normalizedErr.message}`)
      }
    },
  ...
  const handleUpdate = useCallback(
    async (linkId: string, data: SocialLinkFormData): Promise<void> => {
      try {
        ...
        const normalizedErr = err instanceof Error ? err : new Error('Unknown error')
        logger.error({ error: normalizedErr, linkId, data }, 'Failed to update social link')
        showError(`Failed to update link: ${normalizedErr.message}`)
      }
    },
  ...
  const handleDelete = useCallback(
    async (linkId: string): Promise<void> => {
      try {
        ...
        const normalizedErr = err instanceof Error ? err : new Error('Unknown error')
        logger.error({ error: normalizedErr, linkId }, 'Failed to delete social link')
        showError(`Failed to delete link: ${normalizedErr.message}`)
      }
    },
