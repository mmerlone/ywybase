# YwyBase Documentation

Welcome to the YwyBase documentation! This directory contains all the technical documentation for the project.

## Core Documentation

- [Architecture](./architecture.md) - High-level system architecture and design decisions
- [Structure](./structure.md) - Project structure and organization
- [API Documentation](./api.md) - Complete API endpoint documentation
- [Security Documentation](./security.md) - Comprehensive security guide and best practices
- [Rate Limiting](./rate-limiting.md) - API rate limiting and storage setup
- [Authentication Flows](./authentication-flows.md) - Email authentication and verification flows
- [Flash Messages](./flash-messages.md) - Temporary notifications system
- [Database Documentation](./database/) - Database operations and schema management
  - [Profile-Auth Sync](./database/profile-auth-sync-report.md) - Profile-auth synchronization system
  - [Database Recreation](./database/database-recreation.md) - Guide for recreating database
- [Component Documentation](./components/) - UI component library documentation
  - [Avatar Components](./components/avatar.md) - Profile picture management

## Supabase

For Supabase-specific documentation, see the [Supabase directory](../supabase/README.md).

## Contributing

For contribution guidelines, see [CONTRIBUTING.md](../CONTRIBUTING.md).

### Development Resources

Essential resources for contributors:

- **`.windsurf/rules/`** - Comprehensive development guidelines and coding standards
- **`.cascade/ai-context.md`** - Core development patterns and architectural context

When adding new documentation:

1. Use lowercase with hyphens for filenames (e.g., `new-document.md`)
2. Update this README with links to new documentation
3. Keep documentation up-to-date with the codebase

---

**Last Updated**: March 6, 2026
**Version**: 1.0.0
